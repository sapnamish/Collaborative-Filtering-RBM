    ...
    for j in range(epochs):
        ...
        for batch_i, batch in enumerate(chunker(profiles.keys(),
            ...
            train_batch_a = np.array(age_batch).reshape(size,
                                                         len(all_ages))
            train_batch_g = np.array(genre_batch).reshape(size,
                                                         len(all_movies) * len(all_genres))
            ...
            train(train_batch_x, train_batch_o, train_batch_s, train_batch_a, train_batch_g, 
                train_masks_x, train_masks_o, train_masks_s, train_masks_a, train_masks_g)
            sys.stdout.write('.')
            sys.stdout.flush()
        ...
        for batch in chunker(tests.keys(), batch_size):
            ...
            test_batch_a = np.array(age_batch).reshape(size,
                                                        len(all_ages))
            test_batch_g = np.array(genre_batch).reshape(size,
                                                        len(all_movies) * len(all_genres))
...
if __name__ == "__main__":
    ...
    for experiment in experiments:
        ...
        all_users, all_movies, all_occupations, all_sex, all_ages, all_genres, tests = load_dataset(train_path, test_path, user_path, item_path
                                                    sep, user_based=True)

        for config in configs:
            run(name, train_path, user_path, item_path, config, all_users, all_movies, all_occupations, all_sex, all_ages, all_genres, tests,
                None, sep)
